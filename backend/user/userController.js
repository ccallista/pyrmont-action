...
module.exports = {
    ...
    async login(req, res, db) {
        try{
            ...
            access_token = await userAuth.generateAccessToken(userEmailInput);
            refresh_token = await userAuth.generateRefreshToken(userEmailInput);

            return res.status(200).json({message: 'Successfully searched', accessToken: access_token, refreshToken: refresh_token});
